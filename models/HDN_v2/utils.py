...
def build_loss_bbox(bbox_pred, roi_data, fg_cnt):
    ...
    loss_box = F.smooth_l1_loss(
        bbox_pred, bbox_targets, size_average=False) / (fg_cnt + 1e-5)
    return loss_box
...
def interpret_relationships(cls_prob, bbox_pred, rois, cls_prob_predicate,
    ...
    predicate_scores, predicate_inds = cls_prob_predicate[:, 1:].data.topk(
        dim=1, k=topk)
    predicate_inds += 1
    predicate_scores, predicate_inds = predicate_scores.cpu(
    ).numpy().reshape(-1), predicate_inds.cpu().numpy().reshape(-1)
    ...
    if use_gt_boxes:
        ...
        if nms > 0. and pred_boxes.shape[0] > 0:
            ...
            pred_boxes, scores, inds, keep = nms_detections(
                pred_boxes, scores, nms, inds=inds)
    ...
    pred_list = np.vstack(
        [pred_list * topk + i for i in range(topk)]).transpose().reshape(-1)
    sub_list = np.vstack([sub_list for i in range(topk)]
                         ).transpose().reshape(-1)
    obj_list = np.vstack([obj_list for i in range(topk)]
                         ).transpose().reshape(-1)
...
def interpret_objects(cls_prob, bbox_pred, rois, im_info, nms_thres=-1., min_score=0.00001, use_gt_boxes=False, max_per_image=2000):
    ...
    for j in xrange(1, cls_prob.shape[1]):  # skip the background
        ...
        if use_gt_boxes:
            ...
            t_box_deltas = np.asarray(
                [box_deltas[i, (j * 4): (j * 4 + 4)] for i in inds], dtype=np.float)
            cls_boxes = bbox_transform_inv_hdn(
                rois.data.cpu().numpy()[inds, 1:5], t_box_deltas) / im_info[0][2]
    ...
    if max_per_image > 0:
        image_scores = np.hstack(
            [all_boxes[j][:, -1] for j in xrange(1, cls_prob.shape[1]) if len(all_boxes[j]) > 0])
...
def build_loss_reconstruct(gen, raw):
    assert gen.size(0) == raw.size(0)
    assert gen.size(1) == raw.size(1)
    if gen.size()[-2:] != raw.size()[-2:]:
        raw = nn.Upsample(size=gen.size()[-2:], mode='nearest')(raw)

    return torch.mean(torch.abs(gen - raw))

def build_loss_GAN(res):
    num = res.size(0)
    gt = torch.zeros(num, 1).cuda()
    gt[:num//2] = 1

    return nn.BCELoss()(res, gt)
