...
def build_loss(rpn_cls_score_reshape, rpn_bbox_pred, rpn_data):
    ...
    rpn_label = torch.index_select(rpn_label.cuda(), 0, rpn_keep)
    ...
    rpn_bbox_pred = torch.mul(rpn_bbox_pred, rpn_bbox_inside_weights.cuda())
    rpn_loss_box = F.smooth_l1_loss(rpn_bbox_pred, rpn_bbox_targets.cuda(), size_average=False) /  (fg_cnt + 1e-4)
