...
def get_optimizer(lr, mode, opts, vgg_features_var, rpn_features, hdn_features, concept_net_features, rc_net_features, discriminator_net_features, mps_features=[]):
    ...
    if mode == 0:
        ...
        set_trainable_param(hdn_features, True)
        set_trainable_param(concept_net_features, True)
        set_trainable_param(rc_net_features, True)
        set_trainable_param(discriminator_net_features, True)
        if opts['optim']['optimizer'] == 0:
            optimizer = torch.optim.SGD([
                {'params': rpn_features},
                {'params': vgg_features_var, 'lr': lr * 0.1},
                {'params': concept_net_features, 'lr': lr * 0.1},
                {'params': rc_net_features, 'lr': lr * 0.1},
                {'params': discriminator_net_features, 'lr': lr * 0.1},
                {'params': hdn_features},
                ], lr=lr, momentum=opts['optim']['momentum'], weight_decay=0.0005, nesterov=opts['optim']['nesterov'])
        elif opts['optim']['optimizer'] == 1:
            optimizer = torch.optim.Adam([
